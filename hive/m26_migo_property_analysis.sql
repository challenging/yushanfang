...
(
    ...
    SELECT  seller_id,
            ...
            item_pro_value_des,
            trade_amt_item,
            (trade_amt_item/all_trade_amt) AS item_pro_amt_pct,
            0 AS item_GDP_pct
    FROM
    (
        SELECT  m.seller_id,
                m.item_pro_value,
                m.item_pro_value_des,
                m.all_trade_amt,
                SUM(trade_amt_itempro) AS trade_amt_item
        FROM 
        (  -- mapping total amt of seller_id
            SELECT  j.seller_id,
                    j.item_pro_value,
                    j.item_pro_value_des,
                    j.trade_amt_itempro,
                    k.all_trade_amt
            FROM
            (
                ...
                SELECT  seller_id,
                        ...
                        item_pro_value_des,
                        item_trade_amt,
                        SUM(alipay_trade_amt) AS trade_amt_itempro
                        --alipay_trade_amt/all_trade_amt as avg_trade_amt      
                FROM
                ( --get mapping table already
                    SELECT  g.seller_id,
                            h.auction_id,
                            h.alipay_trade_amt,
                            g.item_trade_amt,
                            g.item_pro_value,
                            g.item_pro_value_des
                    FROM
                    ( --get amt by order by item
                        SELECT  seller_id,
                                auction_id,
                                alipay_trade_amt
                    ...
                    (
                        SELECT  e.seller_id, 
                                e.auction_id,
                                f.item_pro_value, 
                                f.item_pro_value_des,
                                e.item_trade_amt
                        FROM 
                        ( --get item in valid orders for property mapping
                            SELECT  seller_id, 
                                    auction_id, 
                                    SUM(alipay_trade_amt) AS  item_trade_amt
                            FROM    sys_seller_dwb_shop_order_d
                            WHERE   trade_status &2 = 2
                                AND dt >=  to_char(dateadd(to_date('${date_ymd}','yyyymmdd'), -30, 'dd'), 'yyyymmdd')
                                AND dt <= '${date_ymd}'  
                        ...
                        (
                            ...
                            SELECT  b.seller_id,
                                    b.auction_id,
                                    b.item_pro_id,
                                    c.property_name AS item_pro_id_des,
                                    b.item_pro_value,
                                    d.property_value_data AS item_pro_value_des
                            FROM 
                            (
                                SELECT  --a.var0, 
                                ...
                                (   
                                    ...
                                    (
                                        SELECT  seller_id , 
                                                auction_id, 
                                                item_property_set, 
                                                CONCAT(seller_id, '_', auction_id) AS sid_aid
                            ...
                            ON b.item_pro_id = c.property_id
                            LEFT OUTER JOIN (
                            ...
                            ON b.item_pro_value = d.property_value_id
                        )f
                        ON e.seller_id = f.seller_id AND e.auction_id = f.auction_id
                    )g
                    ON h.seller_id = g.seller_id AND h.auction_id = g.auction_id
                )i
                GROUP BY seller_id, item_pro_value,  item_pro_value_des, item_trade_amt --auction_id, 
            )j
            LEFT OUTER JOIN
            (   -- total amt by seller_id
                SELECT  seller_id, 
...
ON o.seller_id = p.seller_id AND o.item_pro_value = p.item_pro_value
